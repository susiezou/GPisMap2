        ...
        for (int j = k; j < n; j++) {
            if (k == j) {
                ...
                if (gradflag[k] > -0.5) {
                    ...
                    K(kind1, kind1) = sigx(k) * sqr3L2 + siggrad(k);;
                    K(kind1, kind2) = 0.0;
                    K(kind1, kind3) = 0.0;
                    K(kind2, kind1) = 0.0;
                    K(kind2, kind2) = sigx(k) * sqr3L2 + siggrad(k);
                    K(kind2, kind3) = 0.0;
                    K(kind3, kind1) = 0.0;
                    K(kind3, kind2) = 0.0;
                    K(kind3, kind3) = sigx(k) * sqr3L2 + siggrad(k);
                }
            }
            else {
                float varp = sqrt(sigx(k)) * sqrt(sigx(j));
                float r = (x1.col(k) - x1.col(j)).norm();
                K(k, j) =  varp * kf(r, sqr3L);
                K(j, k) = K(k, j);
                if (gradflag[k] > -1) {

                    K(kind1, j) = -varp*kf1(r, x1(0, k) - x1(0, j), sqr3L);
                    K(j, kind1) = K(kind1, j);
                    K(kind2, j) = -varp*kf1(r, x1(1, k) - x1(1, j), sqr3L);
                    K(j, kind2) = K(kind2, j);
                    K(kind3, j) = -varp*kf1(r, x1(2, k) - x1(2, j), sqr3L);
                    K(j, kind3) = K(kind3, j);
                    ...
                    if (gradflag[j] > -1) {
                        ...
                        K(kind1, jind1) = varp*kf2(r, x1(0, k) - x1(0, j), x1(0, k) - x1(0, j), 1.0, sqr3L);
                        K(jind1, kind1) = K(kind1, jind1);
                        K(kind1, jind2) = varp*kf2(r, x1(0, k) - x1(0, j), x1(1, k) - x1(1, j), 0.0, sqr3L);
                        K(jind1, kind2) = K(kind1, jind2);
                        K(kind1, jind3) = varp*kf2(r, x1(0, k) - x1(0, j), x1(2, k) - x1(2, j), 0.0, sqr3L);
                        K(jind1, kind3) = K(kind1, jind3);

                        K(kind2, jind1) = K(kind1, jind2);
                        K(jind2, kind1) = K(kind1, jind2);
                        K(kind2, jind2) = varp*kf2(r, x1(1, k) - x1(1, j), x1(1, k) - x1(1, j), 1.0, sqr3L);
                        K(jind2, kind2) = K(kind2, jind2);
                        K(kind2, jind3) = varp*kf2(r, x1(1, k) - x1(1, j), x1(2, k) - x1(2, j), 0.0, sqr3L);
                        K(jind2, kind3) = K(kind2, jind3);
                        ...
                        K(kind3, jind2) = K(kind2, jind3);
                        K(jind3, kind2) = K(kind2, jind3);
                        K(kind3, jind3) = varp*kf2(r, x1(2, k) - x1(2, j), x1(2, k) - x1(2, j), 1.0, sqr3L);
                        K(jind3, kind3) = K(kind3, jind3);
                    }
                }
                else if (gradflag[j] > -1) {
                    ...
                    int jind2 = jind1 + ng;
                    int jind3 = jind2 + ng;
                    K(k, jind1) = varp*kf1(r, x1(0, k) - x1(0, j), sqr3L);
                    K(jind1, k) = K(k, jind1);
                    K(k, jind2) = varp*kf1(r, x1(1, k) - x1(1, j), sqr3L);
                    K(jind2, k) = K(k, jind2);
                    K(k, jind3) = varp*kf1(r, x1(2, k) - x1(2, j), sqr3L);
                    K(jind3, k) = K(k, jind3);
                }
            }
        }
    ...
    EMatrixX const& x2, float scale_param, EMatrixX const& sig1, EMatrixX const& sig2)
{
    ...
    for (int k = 0; k < n; k++) {
        ...
        for (int j = 0; j < m; j++) {
            float r = (x1.col(k) - x2.col(j)).norm();
            float varp = sqrt(sig1(k)) * sqrt(sig2(j));
            K(k, j) = varp*kf(r, sqr3L);
            K(k, j + m) = varp*kf1(r, x1(0, k) - x2(0, j), sqr3L);
            K(k, j + m2) = varp*kf1(r, x1(1, k) - x2(1, j), sqr3L);
            K(k, j + m3) = varp*kf1(r, x1(2, k) - x2(2, j), sqr3L);
            if (gradflag[k] > -0.5) {
                K(kind1, j) = -K(k, j + m);
                K(kind2, j) = -K(k, j + m2);
                K(kind3, j) = -K(k, j + m3);
                K(kind1, j + m) = varp*kf2(r, x1(0, k) - x2(0, j), x1(0, k) - x2(0, j), 1.0, sqr3L);
                K(kind1, j + m2) = varp*kf2(r, x1(0, k) - x2(0, j), x1(1, k) - x2(1, j), 0.0, sqr3L);
                K(kind1, j + m3) = varp*kf2(r, x1(0, k) - x2(0, j), x1(2, k) - x2(2, j), 0.0, sqr3L);
                K(kind2, j + m) = K(kind1, j + m2);
                K(kind2, j + m2) = varp*kf2(r, x1(1, k) - x2(1, j), x1(1, k) - x2(1, j), 1.0, sqr3L);
                K(kind2, j + m3) = varp*kf2(r, x1(1, k) - x2(1, j), x1(2, k) - x2(2, j), 0.0, sqr3L);
                K(kind3, j + m) = K(kind1, j + m3);
                K(kind3, j + m2) = K(kind2, j + m3);
                K(kind3, j + m3) = varp*kf2(r, x1(2, k) - x2(2, j), x1(2, k) - x2(2, j), 1.0, sqr3L);
            }
        }
    }
